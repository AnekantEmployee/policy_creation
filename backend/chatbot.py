...
# API_BASE should be set via environment or read from .env
# For Streamlit local development, defaults to localhost:8001
...
def generate_docx(payload: dict, out_path: str) -> str | None:
    try:
        ...
        pol_by_fw  = defaultdict(list)
        proc_by_fw = defaultdict(list)
            ...
            for fw in all_fw:
                ...
                for pol in pol_by_fw[fw]:
                    ...
                    for sec in pol.get("sections", []):
                        ...
                        # Remove any trailing "References:" block the LLM embeds in content
            ...
            with st.expander(f"🏢 {s['org_name']}  ·  {s['created_at'][:10]}  ·  {s['policy_count']} policies  ·  {s['procedure_count']} procedures", expanded=False):
                ...
                if st.session_state.get("history_detail_id") == s["session_id"]:
                    ...
                    if err3:
                        ...
                        if detail["policies"] or detail["procedures"]:
                            ...
                            if not docx_err and os.path.exists(docx_out):
                                with open(docx_out, "rb") as f:
                                    docx_bytes = f.read()
                                os.unlink(docx_out)
                                st.download_button("📄 Download Full Compliance Document (.docx)", data=docx_bytes,
                        ...
                        if detail["policies"]:
                            seen_pol = set()
                            unique_pols = [p for p in detail["policies"] if p.get("policy_id") not in seen_pol and not seen_pol.add(p.get("policy_id"))]
                            st.markdown(f"**Policies ({len(unique_pols)})**")
                            for pi, p in enumerate(unique_pols):
                                fw_tag = f" · {p.get('framework','')}" if p.get('framework') else ''
                                with st.expander(f"📄 {p['title']}{fw_tag}", expanded=False):
                                    render_policy(p, f"dl_pol_{s['session_id']}_{pi}")
                        if detail.get("procedures"):
                            seen_proc = set()
                            unique_procs = [p for p in detail["procedures"] if p.get("procedure_id") not in seen_proc and not seen_proc.add(p.get("procedure_id"))]
                            st.markdown(f"**Procedures ({len(unique_procs)})**")
                            for pi, proc in enumerate(unique_procs):
                                fw_tag = f" · {proc.get('framework','')}" if proc.get('framework') else ''
                                with st.expander(f"📋 {proc['title']}{fw_tag}", expanded=False):
                                    h = html
                                    st.markdown(f"""
                                    <div class="doc-card">
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if pol_data and pol_data.get("policies"):
        ...
        for pi, policy in enumerate(pol_data["policies"]):
            fw_tag = f" · {policy.get('framework','')}" if policy.get('framework') else ''
            with st.expander(f"📄 {policy['title']}{fw_tag}", expanded=False):
